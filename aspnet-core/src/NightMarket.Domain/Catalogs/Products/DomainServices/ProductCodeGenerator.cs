...
namespace NightMarket.Catalogs.Products.DomainServices
{
    public class ProductCodeGenerator : ITransientDependency
    {
        ...
        {
            string newCode;
            var identitySetting = await _identitySettingRepository.FindAsync(NightMarketConsts.ProductIdentitySettingId);
            if (identitySetting == null)
            {
                identitySetting = await _identitySettingRepository.InsertAsync(new IdentitySetting(NightMarketConsts.ProductIdentitySettingId, "Sản phẩm", NightMarketConsts.ProductIdentitySettingPrefix, 1, 1));
